    ...
    with col2:
        ...
        if item_scores:
            critical_items = sorted(
                [(k, v['mean']) for k, v in item_scores.items() if v['mean'] <= score_filter if score_filter > 0 else True],
                key=lambda x: x[1]
            )[:20] if score_filter > 0 else sorted(item_scores.items(), key=lambda x: x[1]['mean'])[:20]
