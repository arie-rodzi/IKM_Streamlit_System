...
class IKMDashboardAdvanced:
    ...
    def get_theory_analysis(self):
        """Build a summary dict of theories from theory_mapping sheet or questionnaire_master."""
        # Prefer dedicated theory_mapping sheet
        ...
        for theory, group in df.groupby('Theory'):
            dimensions = (
                group['Dimension'].dropna().unique().tolist()
                if 'Dimension' in group.columns else []
            )
            subdimensions = (
                group['Subdimension'].dropna().unique().tolist()
                if 'Subdimension' in group.columns else []
            )
            theories[theory] = {
                'count': len(group),
...
def tab_06_qualitative_intelligence():
    ...
    with col1:
        available_cols = st.session_state.dashboard.qualitative_response.columns.tolist()
        default_cols = [col for col in ['Respondent_ID', 'Q1_Main_Concern', 'Q3_Main_Source_Tension', 'State'] 
                       if col in available_cols]
        
        display_cols = st.multiselect(
            "Columns to Display",
            available_cols,
            default=default_cols if default_cols else available_cols[:3]
        )
